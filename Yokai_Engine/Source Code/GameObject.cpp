...
GameObject::GameObject(const char* name, GameObject* parent, int _uid) : parent(parent), name(name), showChildren(false),
...
Component* GameObject::GetComponent(Component::Type type, const char* scriptName)
{
	for (size_t i = 0; i < components.size(); i++)
	{
